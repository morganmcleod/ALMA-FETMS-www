...
require_once($site_classes . '/class.ifspectrumplotter2.php');
...
class IFSpectrumPlotter extends TestData_header {
    var $plotter;                 //class IFSpectrumPlotter2
    ...
    public function Initialize_IFSpectrum($FEid, $dataSetGroup, $fc, $band){
        ...
        $this->TDHdataLabels[] = $this->TDHkeyString;
    ...
    public function GeneratePlots(){
        ...
        $this->Plot_PowerVariation_Data(60, 5);
    ...
    public function makeImageFilenames($typeUrl, $ifChannel = "") {
        ...
        $this->imagename = $typeUrl . "_Band$this->band" . date('Y_m_d_H_i_s') . "dsg" . $this->dataSetGroup . "if" . $ifChannel . ".png";
        $this->imagepath = $imagedirectory . $imagename;
        $this->image_url = $this->url_directory . "tdh/" . $this->TDHkeys[0] . "/IFSpectrum/$imagename";
    }
    ...
    public function Plot_IFSpectrum_Data($expanded, $progressStart, $progressIncrement) {
        ...
        for ($ifChannel=0; $ifChannel<=$iflim; $ifChannel++) {
            ...
            $this->calc->IFChannel = $ifChannel;
            $this->calc->getSpuriousData();
    ...
    public function Plot_PowerVariation_Data($win31MHz, $progressStart, $progressIncrement){
        ...
        for ($ifChannel=0; $ifChannel<=$iflim; $ifChannel++) {
            ...
            // Get spurious noise data from database:
            $this->calc->IFChannel = $ifChannel;
            $this->calc->getPowerData($fwin); // Gets power variation from database for 2 GHz window

            // Set data into the plotter:
            $this->plotter->setData($this->ifCalc->data);
            $plotter->save_data("PowerVarBand$band" . "_$winText" . "_IF$ifChannel");

            // Set the plot title and generate the plot:
            $plotTitle = "Power Variation $winText Window FE-$fesn, Band $this->band SN $this->CCASN IF$ifChannel";
            $plotter->generatePowerVarPlot($win31MHz, $this->imagename, $plotTitle, $this->TDHdataLabels);
