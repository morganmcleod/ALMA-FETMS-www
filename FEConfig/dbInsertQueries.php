...
class dbInsertQueries
{
    function InsertIntoFrontEnds($FEarray)
    {
        ...
        $insert_frontends=mysqli_query($link, "INSERT INTO Front_Ends(SN,ESN,Docs,keyFacility,Description)
    ...
    {
        //called from AddFrontEnd.php
        $insert_configLink=mysqli_query($link, "INSERT INTO FE_ConfigLink(fkFE_Components,fkFE_Config) VALUES('$comp',
        '$maxkey')")
    ...
    function insertIntoStatLocAndNotes($notes,$updatedby,$status,$location,$link_data,$maxkey,$facility)
    {
        ...
        $insert_notesetc=mysqli_query($link, "INSERT INTO FE_StatusLocationAndNotes(fkFEConfig,
    ...
    function insertIntoFEConfig($maxkey, $fesn,$facility)
    {
        ...
        $insert_feconfig=mysqli_query($link, "INSERT INTO FE_Config(fkFront_Ends,Description,keyFacility)
    ...
    function InsertNewConfig($FrontEndArray,$componentArray,$keyFE)
    {
        ...
        $getFEkey=mysqli_query($link, "SELECT fkFront_Ends FROM FE_Config WHERE keyFEConfig='$keyFE'")
        or die("Could not get feconfig" .mysql_error());
        $fe_key=ADAPT_mysqli_result($getFEkey,0,"fkFront_Ends");

        $getFESN=mysqli_query($link, "SELECT SN FROM Front_Ends WHERE keyFrontEnds='$fe_key'")
        or die("Could not get fesn" .mysql_error());
        $fe_sn=ADAPT_mysqli_result($getFESN,0,"SN");

        $insertNewConfig=mysqli_query($link, "INSERT INTO FE_Config(fkFront_Ends,Description)
                                      VALUES('$fe_key','Cold PAS Config for FE SN $fe_sn')")
        or die("Could not insert new config" .mysql_error());

        $NewFEConfig_query=mysqli_query($link, "SELECT MAX(keyFEConfig) as NewConf FROM FE_Config WHERE fkFront_Ends='$fe_key'")
        ...
        foreach($componentArray as $value)
        {
            if($value != "")
            {
                mysqli_query($link, "INSERT INTO FE_ConfigLink(fkFE_Components,fkFE_Config)
                          VALUES('$value','$newconf')")
                or die("Could not insert data" .mysql_error());
            }
        }

        $update_fetable=mysqli_query($link, "Update Front_Ends SET ESN='$FrontEndArray[cansn]' WHERE keyFrontEnds='$fe_key'");

        return $newconf;
    }
    function insertIntoComponents($component_array)
    {
        $insertIntoComponents=mysqli_query($link, "INSERT INTO FE_Components(fkFE_ComponentType,SN,ESN1,ESN2,
        ...
        $Componentkey_query=mysqli_query($link, "SELECT keyId FROM FE_Components WHERE SN='$component_array[sn]' AND
        ...
        $insertintoStatLoc=mysqli_query($link, "INSERT INTO FE_StatusLocationAndNotes(fkFEComponents,fkLocationNames,
